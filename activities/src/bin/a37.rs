...
#[derive(Debug, Error)]
enum RgbError {
    #[error("hex colors must being with a hash (#)")]
    MissingHash,
    #[error("Failed to parse gex digit: {0}")]
    ParseError(#[from] std::num::ParseIntError),
    #[error("The string needs to be in the format #XXXXXX where X is a valid hexidecimal digit")]
    LengthError,
}
